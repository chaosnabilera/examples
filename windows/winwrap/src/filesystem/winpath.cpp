...
bool WinPath::createDirW(std::wstring& path) {
    std::wstring abspath;
    size_t find_idx = std::wstring::npos;
    ...
    find_idx = abspath.rfind(L'\\');
    find_idx = min(find_idx, abspath.rfind(L'/'));

    if (find_idx != std::wstring::npos) {
        std::wstring subpath = abspath.substr(0, find_idx);
