...
struct WinRegistryValueW {
    ...
    std::vector<std::wstring> toWstringVector();
...
public:
    ...
    WinRegistryW(); // == WinRegistryW(wow6464Key)
    explicit WinRegistryW(wow6464Key);
    explicit WinRegistryW(wow6432Key);
    ~WinRegistryW();

    std::wstring sanitizePath(const std::wstring& reg_path);

    // note that keyExist may give false negative if we don't have enough privilege
    bool keyExist(const std::wstring& key_abspath);
    // if ignore_error == true -> if error occurs at RegEnumKeyExA/RegOpenKeyExA for a subkey, just ignore it and continue
    //                 == false-> if error occurs return false immediately
    bool enumKey(const std::wstring& key_abspath, std::vector<std::wstring>* out_subkey_abspath_list, bool ignore_error = true, bool recursive = false);
    bool createKey(const std::wstring& key_abspath, bool* out_created_new = nullptr);
    // note that removeKey may give false positive if we don't have enough privilege
    bool removeKey(const std::wstring& key_abspath, bool recursive);

    bool valueExist(const std::wstring& key_abspath, const std::wstring& value_name);
    bool enumValue(const std::wstring& key_abspath, std::vector<WinRegistryValueW>* out_value_list);
    bool enumValueName(const std::wstring& key_abspath, std::vector<std::wstring>* out_value_name_list);
    // value_type = REG_DWORD, REG_SZ, REG_BINARY, etc.
    bool setValue(const std::wstring& key_abspath, const std::wstring& value_name, DWORD value_type, const BYTE* data, DWORD data_len);
    bool setValueDWORD(const std::wstring& key_abspath, const std::wstring& value_name, const DWORD value);
    bool setValueQWORD(const std::wstring& key_abspath, const std::wstring& value_name, const unsigned long long value);
    bool setValueSZ(const std::wstring& key_abspath, const std::wstring& value_name, const std::wstring& value);
    bool setValueExpandSZ(const std::wstring& key_abspath, const std::wstring& value_name, const std::wstring& value);
    bool setValueMultiSZ(const std::wstring& key_abspath, const std::wstring& value_name, const std::vector<std::wstring>& value);
    bool getValue(const std::wstring& key_abspath, const std::wstring& value_name, WinRegistryValueW* out_value);
    bool removeValue(const std::wstring& key_abspath, const std::wstring& value_name);

private:
    const bool isWow6464;
    bool disasWinRegAbsPath(const std::wstring& key_abspath, HKEY* out_hkey_root, std::wstring* out_subroot_path);
    bool recEnumKey(HKEY hkey, std::wstring prefix_path, std::vector<std::wstring>* out_subkey_abspath_list, bool ignore_error, bool recursive);
    bool openKeyHandle(const std::wstring& key_abspath, DWORD sam_desired, HKEY* out_key_handle);
};
